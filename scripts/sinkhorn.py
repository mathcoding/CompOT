...
def MinRow(A, eps):
    n, m = A.shape
    return np.array([MinEps(A[i,:], eps) for i in range(n)])
    
def MinCol(A, eps):
    n, m = A.shape
    return np.array([MinEps(A[:,j], eps) for j in range(m)])
...
def LogSinkhorn(a, b, C, eps=0.1, maxit=100):
    ...
    for i in range(maxit):
        #f = eloga - eps*np.log(K @ np.exp(g0/eps))
        #g = elogb - eps*np.log(K.T @ np.exp(f/eps))
        
        # f = np.zeros(n)
        # for i in range(n):
        #     # (4.38) in Cuturi's book
        #     f[i] = eloga[i] + MinEps(C[i,:] - g0, eps)

        # g = np.zeros(m)
        # for j in range(m):
        #     # (4.39) in Cuturi's book
        #     g[j] = elogb[j] + MinEps(C[:,j] - f, eps)
        
        f = eloga + MinRow(C - np.outer(np.ones(n), g0), eps)
        g = elogb + MinCol(C - np.outer(f, np.ones(m)), eps)

        #Sfg = np.matrix([[(C[i,j] - f0[i] - g0[j]) for i in range(n)] for j in range(m)])
        #f = eloga + f0 + MinRow(Sfg, eps)

        #Sfg = np.matrix([[(C[i,j] - f[i] - g0[j]) for i in range(n)] for j in range(m)])
        #g = elogb + g0 + MinCol(Sfg, eps)
        ...
        if i % 10  == 0:
            ...
            Pobj = np.sum(C * P) + eps * np.sum(P * np.log(P)- P)
            Dobj = eps*(np.sum(a * np.log(u)) + np.sum(b * np.log(v))) - eps*(u.T@(K@v))
            
            print(i, 'Pobj:', round(Pobj, 6), 'Dobj:', round(Dobj, 6))
            # Primal dual gap
            if abs(Pobj - Dobj) < 1e-9:
                break
            # Obj stalling
            if abs(Pobj - P0) < 1e-9 and abs(Dobj - D0) < 1e-9:
                print('stalling')
                break
            P0, D0 = Pobj, Dobj
        f0 = f[:]
    ...
    v = np.exp(g/eps)
    P = np.diag(u) @ K @ np.diag(v)
    fobj = np.sum(C * P) + eps * np.sum(P * np.log(P)-P)
    return fobj, P
...
if __name__ == "__main__":
    ...
    if Test == 1:
        ...
        #print(P,  np.sum(C * P) + 0.1 * np.sum(P * np.log(P)-P))
